...
export function LoginForm() {
  ...
  if (status === "sent") {
    return (
      <p className="font-body text-body text-ink-secondary leading-relaxed">
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-4">
      <label className="font-body text-small text-ink-secondary flex flex-col gap-2">
        Email address
        <input
          type="email"
          required
      ...
      {status === "error" && (
        <p className="font-body text-small text-ink-secondary">{message}</p>
      )}
    </form>
  );
}
